...
- Em 24/08 a cota de mutations estourou de novo (1.007/1.000), um dia depois da
  ...
  em paralelo; quando o cookie de ~30 min morria, a rajada inteira relogava e
  cada request gravava a sessão nova (e logins concorrentes derrubam a sessão
  de novo — espiral). Correção: poll sequencial round-robin no cliente (≤ 1
  request em voo por dispositivo), que tampa a escrita em ~1 por morte de
  cookie. Lição: no hospedado, o multiplicador de mutations é a concorrência
  de requests, não a frequência — qualquer caminho que escreva no DB precisa
  ser à prova de rajada.
